...
def Grand_h(event=None):
    global MonJeu
    if not FJeu.fin(MonJeu, JeuDepart):
        MonJeu = FJeu.rotation(MonJeu,Matrice_Mouv[mes_dir[0][1]])
        change1(MonJeu)

def Grand_t(event=None):
    global MonJeu
    if not FJeu.fin(MonJeu, JeuDepart):
        MonJeu = FJeu.rotation(MonJeu,Matrice_Mouv[mes_dir[0][0]])
        change1(MonJeu)
def Petit_h(event=None):
    global MonJeu
    if not FJeu.fin(MonJeu, JeuDepart):
        MonJeu = FJeu.rotation(MonJeu,Matrice_Mouv[mes_dir[1][1]])
        change1(MonJeu)

def Petit_t(event=None):
    global MonJeu
    if not FJeu.fin(MonJeu, JeuDepart):
        MonJeu = FJeu.rotation(MonJeu,Matrice_Mouv[mes_dir[1][0]])
        change1(MonJeu)


root.bind('<Key-z>', Grand_h)
root.bind('<Key-s>', Grand_t)
root.bind('<Key-q>', Petit_h)
root.bind('<Key-d>', Petit_t)
